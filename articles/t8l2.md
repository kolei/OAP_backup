...
def js_print(browser, lang, event, msg):
    """запускает Javascript-функцию  "js_print" в браузере (с помощью функции browser.ExecuteFunction мы можем вызвать любую Javascript-функцию из нашей странички)"""
...
```py
    ...
    bindings = cef.JavascriptBindings(
            bindToFrames=False, bindToPopups=False)
    # в объект привязок добавляем аттрибуты
    # python_property - просто текст
    bindings.SetProperty("python_property", "This property was set in Python")
    # cefpython_version - в аттрибут сохранится результат выполнения функции cef.GetVersion()
    bindings.SetProperty("cefpython_version", cef.GetVersion())
...
4. Можно посылать javascript callback-функции в Питон, просто добавьте javascript-функцию как аргумент при вызове функции/объекта Питона:
```js
//из javascript вызываем функцию Питона html_to_data_uri с сообщением "test" и callback-функцией js_callback_1
html_to_data_uri("test", js_callback_1);
```
